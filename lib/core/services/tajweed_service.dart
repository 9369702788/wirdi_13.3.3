...
class TajweedService {
  ...
  /// Same as [analyze], but never throws -- falls back to the
  /// original text as one unstyled segment on any unexpected error.
  static List<TajweedSegment> safeAnalyze(String text) {
    try {
      return analyze(text);
    } catch (_) {
      return [TajweedSegment(text, TajweedRule.none)];
    }
  }
